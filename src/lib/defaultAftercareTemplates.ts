/** Section content for aftercare emails — stored as JSON in shop_aftercare_templates. */
export type AftercareSection = {
  title: string;
  listItems?: string[];
  orderedList?: boolean;
  /** Plain text or simple HTML paragraphs for non-list sections */
  bodyHtml?: string;
};

export type AftercareTemplateContent = {
  badge: string;
  title: string;
  emailSubject: string;
...
export type ShopAftercareTemplate = AftercareTemplateContent & {
  kind: AftercareKind;
  enabled: boolean;
};
...
export const DEFAULT_TATTOO_AFTERCARE: AftercareTemplateContent = {
  badge: "Tattoo aftercare",
  title: "Tattoo aftercare guide",
  emailSubject: "Tattoo Aftercare",
  introTemplate:
  ...
  sections: [
    ...
    },
    {
      ...
      bodyHtml: infectionBody,
    },
  ],
};

export const DEFAULT_PIERCING_AFTERCARE: AftercareTemplateContent = {
  badge: "Piercing aftercare",
  title: "Piercing aftercare guide",
  emailSubject: "Piercing Aftercare",
  introTemplate:
...
export function defaultAftercareForKind(kind: AftercareKind): AftercareTemplateContent {
  return kind === "piercing" ? { ...DEFAULT_PIERCING_AFTERCARE } : { ...DEFAULT_TATTOO_AFTERCARE };
}

export function defaultShopAftercareTemplates(): ShopAftercareTemplate[] {
  return [
    { kind: "tattoo", enabled: true, ...DEFAULT_TATTOO_AFTERCARE },
    { kind: "piercing", enabled: true, ...DEFAULT_PIERCING_AFTERCARE },
  ];
}
