...
import { DOC_CATEGORIES, DOC_PAGES, getDocBySlug, getDocsByCategory } from "@/lib/docsContent";
  ...
  <div>
    ...
    <div className="mt-12 space-y-10">
      {DOC_CATEGORIES.map((cat) => {
        ...
        return (
          <div key={cat.id}>
            <h2 className="text-xs font-semibold uppercase tracking-wider text-[#d4af37]/80">{cat.label}</h2>
          ...
          {!slug ? (
            ...
            <>
              ...
              <div className="mt-14 flex flex-wrap gap-4 border-t border-border/50 pt-8 text-sm">
                {DOC_PAGES.map((p, i) => p.slug === page.slug && DOC_PAGES[i + 1] ? (
                  <Link
                    key={p.slug}
                    to={`/docs/${DOC_PAGES[i + 1].slug}`}
                    className="text-[#d4af37] hover:underline"
                  >
                    Next: {DOC_PAGES[i + 1].title} →
                  </Link>
                ) : null)}
              </div>
            </>
