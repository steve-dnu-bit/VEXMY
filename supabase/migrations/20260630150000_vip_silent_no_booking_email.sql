-- VIP is internal studio metadata (deposit waiver). Toggling it must not email the client.
...
COMMENT ON FUNCTION public.set_client_vip_for_organization(uuid, uuid, text, text, text, boolean) IS
  'Sets vip_client on all matching client bookings in an org. Does not trigger booking update emails.';
