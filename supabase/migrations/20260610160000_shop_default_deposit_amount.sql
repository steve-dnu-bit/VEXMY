-- Shop-wide default deposit amount (shop owner configurable, max 200 in shop currency).

ALTER TABLE public.shop_settings
  ADD COLUMN IF NOT EXISTS default_deposit_amount numeric(10,2) NOT NULL DEFAULT 50.00;

COMMENT ON COLUMN public.shop_settings.default_deposit_amount IS
  'Default deposit for new bookings. Shop owner may set up to 200 (shop currency).';

ALTER TABLE public.shop_settings
  DROP CONSTRAINT IF EXISTS shop_settings_default_deposit_amount_range;

ALTER TABLE public.shop_settings
  ADD CONSTRAINT shop_settings_default_deposit_amount_range
  CHECK (default_deposit_amount >= 0.30 AND default_deposit_amount <= 200);
...
BEGIN
  ...
  SELECT ss.default_deposit_amount INTO v_default
  FROM public.shop_settings ss
  WHERE ss.organization_id = v_org_id
  LIMIT 1;

  v_deposit := public._valid_deposit_amount(COALESCE(p_deposit_amount, v_default, 50));
...
BEGIN
  ...
  SET
    ...
    deposit_amount = CASE
      WHEN p_patch ? 'deposit_amount' THEN public._valid_deposit_amount((p_patch->>'deposit_amount')::numeric)
...
REVOKE ALL ON FUNCTION public.staff_insert_booking(
  uuid, text, text, text, uuid, text, text, text, text, text, text, boolean, timestamptz, timestamptz, text, numeric
) FROM PUBLIC;
